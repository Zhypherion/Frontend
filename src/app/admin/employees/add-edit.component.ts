...
//   next: (positions: any[]) => {
//     this.positions = positions.filter(p => p.status === 'Active');
//   },
...
//                 positionId: x.position?.id ? String(x.position.id) : '',
...
//       payload = { ...raw, employeeId, positionId: Number(raw.positionId) };
...
export class AddEditComponent implements OnInit, OnDestroy {
  ...
  departments: any[] = [];
  positions: any[] = [];
  filteredPositions: any[] = []; // ✅ new filtered list
  ...
  ngOnInit() {
    ...
    this.positionService.getAll().pipe(first()).subscribe({
      next: (positions: any[]) => {
        this.positions = positions.filter(p => p.status === 'Active');
        this.filteredPositions = [...this.positions]; // ✅ initially show all
      },
      error: err => console.error('Error loading positions', err)
    });
    ...
    this.routeSub = this.route.params.subscribe(params => {
        ...
        this.employeeService.getById(this.id!)
          ...
          .subscribe({
            next: (x: any) => {
              ...
              // ✅ Filter positions immediately when editing
              this.filterPositionsByDepartment(x.departmentId);
            },
            error: err => {
              console.error('Error loading employee', err);
              this.alertService.error('Could not load employee record');
      ...
      // ✅ Watch department changes and filter positions dynamically
      this.form.get('departmentId')?.valueChanges.subscribe(deptId => {
        this.filterPositionsByDepartment(deptId);
  ...
  // ✅ Helper function for filtering positions by department
  private filterPositionsByDepartment(deptId: string | number) {
    if (!deptId) {
      this.filteredPositions = [...this.positions];
      return;
    }
    this.filteredPositions = this.positions.filter(p => String(p.departmentId) === String(deptId));
  }
